...
명령줄 인자를 통해 실행 모드 및 가속 방식을 선택할 수 있습니다.

### ⚡ GPU 가속 (`--gpu`)
모든 모드(기본 CLI, TUI, 서버)에서 `--gpu` 옵션을 붙이면 GPU 백엔드 가속을 사용합니다.
```bash
# GPU 가속으로 TUI 모드 실행
./bazel-bin/multimodal_cli --ui --gpu <모델경로>
```

### 1. 인터랙티브 CLI 모드 (기본)
가장 간단한 텍스트 기반 대화 모드입니다.
```bash
./bazel-bin/multimodal_cli <모델경로>
```

### 2. TUI 모드 (`--ui`)
화면 전체를 사용하는 그래픽 터미널 인터페이스입니다.
```bash
./bazel-bin/multimodal_cli --ui <모델경로>
```
- **F1**: 채팅 탭으로 이동
...
```bash
./bazel-bin/multimodal_cli --server --port 11434 <모델경로>
```
